...
class DatePickerModel extends CommonPickerModel {
  ...
  DatePickerModel(
      {DateTime? currentTimeParam,
      DateTime? maxTimeParam,
      DateTime? minTimeParam,
      super.locale})
      : maxTime = maxTimeParam ?? DateTime(2049, 12, 31),
        minTime = minTimeParam ?? DateTime(1970, 1, 1),
        super() {
    currentTime = _calculateCurrentTime(
        currentTimeParam, maxTimeParam, minTimeParam);
    _fillLeftLists();
    _fillMiddleLists();
    _fillRightLists();
    int minMonth = _minMonthOfCurrentYear();
    int minDay = _minDayOfCurrentMonth();
    _currentLeftIndex = currentTime.year - minTime.year;
    _currentMiddleIndex = currentTime.month - minMonth;
    _currentRightIndex = currentTime.day - minDay;
  }

  static DateTime _calculateCurrentTime(
      DateTime? currentTime, DateTime? maxTime, DateTime? minTime) {
    final max = maxTime ?? DateTime(2049, 12, 31);
    final min = minTime ?? DateTime(1970, 1, 1);
